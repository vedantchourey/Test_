...
const getNotificationObj = (
  knexConnection: Knex
): CrudRepository<INotifications> => {
  return new CrudRepository<INotifications>(
    knexConnection,
    TABLE_NAMES.NOTIFICATIONS
  );
};
export const addNotifications = async (
  data: INotifications | INotifications[],
  knexConnection: Knex
): Promise<void> => {
...
export const submitNotifications = async (
  ...
  try {
    ...
    if (!notification)
      return getErrorObject(
        "Invalid notification id or notification response already submitted"
      );

    const resp = await handleNotifiction(
      notification,
      req,
      user,
      knexConnection,
...
export const handleNotifiction = async (
  ...
  if (notification.type === "TOURNAMENT_INVITE") {
    const updated = await updateTournamentInvites(
      { status: req.response, gameUniqueId: req.gameUniqueId } as any,
      {
