...
async function fetchLeaderBoard(req: any, knexConnection: Knex) {
    try {
        ...
        const data = await eloRepo.knexObj().where("game_id", req.game_id).orderBy('elo_rating', 'desc').limit(10)
        return data
    } catch {
