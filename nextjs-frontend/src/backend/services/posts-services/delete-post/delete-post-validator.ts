...
async function validatePostId(post: IDeletePostRequest, postRepository: PostsRepository, context: PerRequestContext): Promise<unknown> {
  if (!isUUID(post.postId)) return 'Invalid post id';
  const postData = await postRepository.getPostById(post.postId as string);
  if ((postData && postData.postedBy) !== context.user?.id as string) return 'You are not allowed to delete post';
}
