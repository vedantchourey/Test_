...
export default function Followersmodal({username}:any): JSX.Element {
    ...
    return(
        <Box className={styles.userList}>
              <Box sx={{ display: 'flex', alignItems: 'center' }} onClick={()=>{router.push(`account/${username}`)}}>
              {data?.avatarUrl ?(<Image bucket={config.storage.publicBucket} filePath={data?.avatarUrl || ''} isPublicBucket={true} width={50} height={50} className={styles.profileAvatar} />):
