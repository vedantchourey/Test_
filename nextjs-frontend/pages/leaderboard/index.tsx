...
const Leaderboard = (): JSX.Element => {
  ...
  const getleaderboardgamedata = async (gameId: string): Promise<void> => {
    try {
      ...
      axios.get(endpoint, { params: { game_id: gameId }, headers: headers }).then((res) => {
        setData(res.data);
                  ...
                  <Box className={styles.container}>
                    ...
                    <Box style={{ marginLeft: '45px' }}>
                      <Box className={styles.box1}>
                        <Typography className={styles.text1}>{item.user_id}</Typography>
                        <Box className={styles.box2}>
                          <Typography className={styles.text2}>ELO RATING</Typography>
                          <Button variant="text" className={styles.button1}>{item.elo_rating}</Button>
                        </Box>
                        <Box className={styles.box3}>
                          <Box className={styles.box4}>
                            <img src="/images/game8.png" className={styles.img2} />
                          </Box>
                ...
                <TableBody sx={{ '&:last-child td, &:last-child th': { border: 1, borderColor: 'rgba(255, 255, 255, 0.1)' } }}>
                  {leaderboardgamedata.map((item) => (
                    <TableRow key={item.id}>
                      <TableCell align="center" component="th" scope="row">{item.id}<sup>th</sup></TableCell>
                      <TableCell>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          <span><img src="/icons/Ellipse 4.png" /></span>
                          <span style={{ padding: '10px' }}>{item.user_id}</span>
                        </div>
                      </TableCell>
