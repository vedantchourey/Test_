...
import { updateOrderPaymentStatus } from "../../../src/backend/services/order-service/Order-Service";
...
export default createNextJsRouteHandler({
    post: {
        handler: async (
            ...
            const result: any = await updateOrderPaymentStatus(req.body, context)
            res.status(result.errors ? 400 : 200).json(result as any);
        },
