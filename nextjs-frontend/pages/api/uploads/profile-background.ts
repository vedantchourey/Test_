...
import { profileBackgroundImageMiddleware } from '../../../src/backend/utils/api-middle-ware/multi-part-file-upload-middle-ware/multi-part-upload-types';
...
import { ProfileBackgroundFileType } from '../../../src/backend/services/file-service/profile-background-file-type';
...
export default createNextJsRouteHandler({
  post: {
    handler: async (req: NextApiRequest, res: NextApiResponse<ServiceResponse<UploadFileRequest, IFileResponse>>, context: PerRequestContext): Promise<void> => {
      ...
      const result = await uploadFile(multiPartRequest.files, new ProfileBackgroundFileType(context));
      return res.status(result.errors == null ? 200 : 400).send(result);
    },
    preHooks: [beginTransactionMiddleWare, authenticatedUserMiddleware, profileBackgroundImageMiddleware],
