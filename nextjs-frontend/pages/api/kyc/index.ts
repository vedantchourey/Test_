...
import { fetchKYC, verifyKYC } from "../../../src/backend/services/kyc-service";
...
export default createNextJsRouteHandler({
    post: {
        handler: async (
            ...
            const result = await verifyKYC(context.knexConnection as Knex, context.user, req.body)
    ...
    get: {
        handler: async (
            ...
            const result = await fetchKYC(context.knexConnection as Knex, context.user)
            res.status(result?.errors?.length ? 500 : 200).json(result)
        },
        preHooks: [beginTransactionMiddleWare, authenticatedUserMiddleware],
        postHooks: [commitOrRollBackTransactionMiddleWare],
